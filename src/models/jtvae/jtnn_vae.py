...
from src.utils.datautils import moltree_tensorize
...
from src.models.fast_jtnn import MolTree, MPN, JTMPN
from src.models.fast_jtnn.encoder import JTNNEncoder
from src.models.jtvae.decoder import JTNNDecoder
...
class JTNNVAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.jtnn = JTNNEncoder(args.hidden_size, args.depthT, nn.Embedding(args.vocab.size(), args.hidden_size), args.rnn_type)
        self.decoder = JTNNDecoder(args.vocab, args.hidden_size, latent_size, nn.Embedding(args.vocab.size(), args.hidden_size))

        self.jtmpn = JTMPN(args.hidden_size, args.depthG)
        self.mpn = MPN(args.hidden_size, args.depthG)

        self.A_assm = nn.Linear(latent_size, args.hidden_size, bias=False)
        self.assm_loss = nn.CrossEntropyLoss(size_average=False)

        self.T_mean = nn.Linear(args.hidden_size, latent_size)
        self.T_var = nn.Linear(args.hidden_size, latent_size)
        self.G_mean = nn.Linear(args.hidden_size, latent_size)
        self.G_var = nn.Linear(args.hidden_size, latent_size)
    ...
    def encode_from_smiles(self, smiles_list):
        ...
        _, jtenc_holder, mpn_holder = moltree_tensorize(tree_batch, self.vocab, assm=False)
